...
public class lab4 {
    public static void main(String[] args) {
        ...
        if (args.length == 1){
            ...
            while(true){
                ...
                processCommand(emulator, command, instList, simulator);
            }
            input.close();
    ...
    private static void processCommand(Emulator emulator, String command, List<Instruction> instList, Simulator simulator) {
        if(command.equals("h")){
            emulator.h_inst();
            ...
            if(command.equals("s")){
                ...
                simulator.s_inst(emulator, 1, instList);
            }else{
                //emulator.s_inst(Integer.parseInt(command.split(" ")[1]), instList);
                simulator.s_inst(emulator, Integer.parseInt(command.split(" ")[1]), instList);
            }
        }
        else if(command.equals("r")){
            //emulator.r_inst(instList.size(), instList);
            simulator.r_inst(emulator, instList);
        }
        else if(command.startsWith("m")){
            emulator.m_inst(command);
        }
        else if(command.startsWith("p")) {
            simulator.p_inst(emulator);
        }
        else {
