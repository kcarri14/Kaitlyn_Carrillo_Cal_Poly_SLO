...
def processing_thread(connection, ip, port, forwarding_table_with_range, default_gateway_port, max_buffer_size=5120):
    ...
    router3_soc = create_socket("127.0.0.1", 8005)
    router4_soc = create_socket("127.0.0.1", 8006)
    ...
    while True:
        ...
        if new_ttl <= 0 and sending_port != "127.0.0.1":
        ...
        if sending_port == "8005":
            ...
            router3_soc.sendall(new_packet.encode())
