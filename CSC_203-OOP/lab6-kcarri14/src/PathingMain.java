...
public class PathingMain extends PApplet
{
   ...
   private boolean searchForGoal(Point start, Point end, List<Point> path) {
      ...
      Function<Point, Stream<Point>> potentialNeighbors = p -> Stream.of(
              ...
              new Point(p.x - 1, p.y),
              new Point(p.x + 1, p.y)
      ).filter(PathingMain::withinBounds);

      // Call the pathing strategy's computePath
      List<Point> searchedPath = pathingStrategy.computePath(start, end, canPassThrough, withinReach, PathingStrategy.CARDINAL_NEIGHBORS);
