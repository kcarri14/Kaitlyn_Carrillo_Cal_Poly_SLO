...
{
   Header *p, *prevp, *worst_fit = NULL, *worst_fit_prev = NULL;
   size_t nunits;
   ...
   for (p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
      ...
      if (p->s.size >= nunits && p->s.size > largest_size)
      {
        largest_size = p->s.size;
        worst_fit = p;
        worst_fit_prev = prevp;
      }
        if(p == freep){
            break;
        }
   ...
   if(worst_fit){
    if(worst_fit->s.size == nunits){
        worst_fit_prev->s.ptr = worst_fit->s.ptr;
    }else{
        worst_fit->s.size -= nunits;
        worst_fit += worst_fit->s.size;
        worst_fit->s.size = nunits;
    }
    freep = worst_fit_prev;
    return worst_fit + 1;
   }  
   if((p = morecore(nunits)) == NULL){
    return NULL;
   }
   return lab_malloc(nbytes);
