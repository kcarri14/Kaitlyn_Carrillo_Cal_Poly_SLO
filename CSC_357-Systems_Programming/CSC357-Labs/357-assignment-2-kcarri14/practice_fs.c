...
void cd(uint32_t cwd, char *name) {
    ...
    char directoryRead[33]; // Buffer to hold the directory name (33 includes space for null terminator)
    ...
    while (fread(&inode_holder, sizeof(uint32_t), 1, readDirect) == 1) {
        ...
        if (strcmp(directoryRead, name) == 0) {
            ...
            if (inodes[inode_holder].type == 'd') {
                ...
                printf("Changed directory to inode %u (name: %s)\n", inode_holder, name);
                fclose(readDirect);
                return;
            } else {
...
int mkdir(uint32_t cwd,char *name){
    ...
    char curent_old_directory[max_length] = "..";

    fwrite(&inode_index, sizeof(uint32_t), 1, fp);
    fwrite(new_directory, sizeof(char), sizeof(new_directory), fp);

    // add parent to current
	fwrite(&cwd, sizeof(uint32_t), 1, fp);
    fwrite(curent_old_directory, sizeof(char), sizeof(curent_old_directory), fp);
    ...
    parent_fp = fopen(parent_inode, "ab");
    if(!parent_fp) {
    ...
    fwrite(&inode_index, sizeof(uint32_t), 1, parent_fp);
    fwrite(current_name, sizeof(current_name), 1, parent_fp);
