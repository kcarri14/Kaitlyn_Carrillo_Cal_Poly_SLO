...
(define (primop? [a : Sexp]) : Boolean
  (or (equal? a '+) (equal? a '-) (equal? a '*) (equal? a '/)
      ...
      (equal? a 'equal?) (equal? a 'error) (equal? a 'println)
      (equal? a 'read-num) (equal? a 'read-str) (equal? a 'seq) (equal? a '++)))
...
(define (lookup [for : Symbol] [env : Environment]) : Value
  (cond
    [(empty? env) (error 'lookup "SHEQ: name not found ~e" for)]
    [else (cond
...
;this function turns any value into a string so that they can be appeneded to each other
(define (value2string [v : Value])
  (match v
    [(StrV s) s]
    [(NumV n) (number->string n)]
    [(BoolV b) (if b "true" "false")]
    [_ (error 'value2string "SHEQ: couldn't make this into a string since its not a StrV, NumV, or BoolV")]))
...
(define (primv-interp [sym : Symbol] [operands : (Listof Value)]) : Value
  (match (list sym operands)
    ...
    [(list '- (list (NumV a) (NumV b))) (NumV (- a b))]
    [(list '* (list (NumV a) (NumV b))) (NumV (* a b))]
    [(list '** (list (NumV a) (NumV b))) (NumV (cast (expt (cast a Number) (cast b Number)) Real))]
    [(list 'sqrt (list (NumV a) (NumV b))) (if (not (equal? b 2))
                                               (error 'primv-interp "SHEQ: can only do square root (2)")
                                               (NumV (cast (sqrt a) Real)))]
    [(list '// (list (NumV b) (NumV c))) (if (not(equal? c 1))
                                             (error 'primv-interp "SHEQ: bottom number must be 1") (NumV (floor b)))]
    [(list '/ (list (NumV a) (NumV b))) (if (zero? b)
                                            (error 'primv-interp "SHEQ: can't do division by zero") (NumV (/ a b)))]
    [(list '% (list (NumV a) (NumV b))) (if (zero? b)
                                            (error 'primv-interp "SHEQ: can't do modulo by zero")
                                            (NumV (modulo (cast a Integer) (cast b Integer))))]
    ...
    [(list 'println (list (StrV s))) (displayln s) (BoolV #t)]
    ;do i have to output a number instead of string?
    ;how do i test it?
    [(list 'read-num '())
     (display "> ")
     (define line (read-line))
     (if (string? line)
         (match (string->number line)
           [(? real? n) (NumV n)]
           [_ (error 'primv-interp "SHEQ: input for read-num must be a real number")])
         (error 'primv-interp "SHEQ: input for read-num must be a string"))]
    [(list 'read-str '())
     (display "> ")
     (define line (read-line))
     (if (string? line)
         (StrV line)
         (error 'primv-interp "SHEQ: input for read-str must be a string"))]
    
    [(list 'seq a) (last a)]
    [(list '++ args) (define strings (map (lambda (a) (value2string a)) args))
                     (StrV (string-join strings ""))]
...
(define (interp [exp : ExprC] [env : Environment]) : Value
  (match exp
    ...
    [(AppC f a)
     ...
     (define arg-vals (map (lambda ([arg : ExprC]) (interp arg env)) a))
     (match fun
       [(CloV p b e)
        (if (= (length p) (length arg-vals))
            (let ()
              ...
              (interp b call-env))
            (error 'interp "SHEQ: wrong number of arguments ~e" arg-vals))]
...
(check-equal? (interp (AppC (lambdaC '(x y) (AppC (idC '%) (list (idC 'x) (idC 'y))))
                            (list (NumC 10) (NumC 3))) top-env) (NumV 1))
(check-equal? (interp (AppC (lambdaC '(x y) (AppC (idC '**) (list (idC 'x) (idC 'y))))
                            (list (NumC 10) (NumC 3))) top-env) (NumV 1000))
(check-equal? (interp (AppC (lambdaC '(x y) (AppC (idC '//) (list (idC 'x) (idC 'y))))
                            (list (NumC 4.32455654) (NumC 1))) top-env) (NumV 4.0))
(check-equal? (interp (AppC (lambdaC '(x y) (AppC (idC 'sqrt) (list (idC 'x) (idC 'y))))
                            (list (NumC 4) (NumC 2))) top-env) (NumV 2))
(check-equal? (interp (AppC (lambdaC '(x y) (AppC (idC '<=) (list (idC 'x) (idC 'y))))
...
(check-equal? (primv-interp 'println (list (StrV "hello world"))) (BoolV #t))
...
(check-exn (regexp (regexp-quote "SHEQ: bottom number must be 1"))
...
(check-exn (regexp (regexp-quote "SHEQ: can only do square root (2)"))
           (lambda () (interp (AppC (lambdaC '(x y) (AppC (idC 'sqrt) (list (idC 'x) (idC 'y))))
                                    (list (NumC 6) (NumC 0))) top-env)))

(check-exn (regexp (regexp-quote "SHEQ: can't do modulo by zero"))
           (lambda () (interp (AppC (lambdaC '(x y) (AppC (idC '%) (list (idC 'x) (idC 'y))))
                                    (list (NumC 6) (NumC 0))) top-env)))
...
(check-equal? (interp (AppC (idC 'seq)
                            (list (AppC (idC 'println) (list (strC "hi")))
                                  (AppC (idC 'println) (list (strC "testing")))
                                  (AppC (idC 'println) (list (strC "this")))
                                  (NumC 42)))
                      top-env) (NumV 42))

(check-equal? (primv-interp 'println (list(primv-interp '++ (list (NumV 6) (StrV "my") (StrV "name")
                                                                  (StrV "is") (StrV "kaitlyn") (BoolV #t)
                                                                  (BoolV #f))))) (BoolV #t))

(check-equal? (top-interp '{seq {println "this program will be really nifty"}
                                {println "after I write it"}}) "true")

(check-exn (regexp (regexp-quote "SHEQ: couldn't make this into a string since its not a StrV, NumV, or BoolV"))
           (lambda () (value2string (PrimV '+))))
...
(define example-program
  '{let {[num1 = {seq {println "Welcome to Kaitlyn and Jake's Awesome Calculator!"}
                      {println "Rules: "}
                      (println "1. Only use real numbers please! (No sqrt of -1)")
                      {println "2. If dividing and using modulo, second number must not be zero"}
                      {println "3. If you are using the floor function //, please input 1 as your second number"}
        {println "4. No negatives when using square root and the first number is the one getting square rooted"}
                      {println "5. If you are using the square root function sqrt, please input 2 as your number"}
                      {println "Please input First Number"}{read-num}}]
         [num2 = (seq {println "Please input Second Number"}{read-num})]
         [op = {seq {println "Please input Operation (+, -, *, /, %, //, sqrt, **)"}{read-str}}]}
     in
     {let {[f = (if (equal? op "+")
                    (lambda (x y) : (+ x y))
                    (if (equal? op "-")
                                ...
                                (if (equal? op "%")
                                    ...
                                    (if (equal? op "//")
                                        ...
                                        (if (equal? op "sqrt")
                                            ...
                                            (if (equal? op "**")
                                                (lambda (x y) : (** x y))
                                                (error "not an operation. Please pick from +, -, *, /, %, //, sqrt, **")
                                                ))))))))]}
       
       in
       {let {[result = {f num1 num2}]}
         in
         {seq {println {++ "Result: " result}}}
         end}
       end}
     end})

;Text generated during the sample run of the program

;Welcome to Kaitlyn and Jake's Awesome Calculator!
;Rules: 
;1. Only use real numbers please! (No sqrt of -1)
;2. If dividing and using modulo, second number must not be zero
;3. If you are using the floor function //, please input 1 as your second number
;4. No negatives when using square root and the first number is the one getting square rooted
;5. If you are using the square root function sqrt, please input 2 as your number
;Please input First Number
;Please input Second Number
;Please input Operation (+, -, *, /, %, //, sqrt, **)
