...
(define-type ExprC (U NumC strC idC ifC lambdaC AppC MutC))
...
(define (cnbt? [s : Sexp]) : Boolean
  (or (equal? s 'if) (equal? s ':) (equal? s 'let) (equal? s '=) (equal? s 'in) (equal? s 'end) (equal? s ':=)))
...
(define (make-initial-store [memsize : Natural]) : Store
  ...
  (vector-set! store (ann 10 Natural) (PrimV 'equal?))
  (vector-set! store (ann 11 Natural) (PrimV 'error))
  (vector-set! store (ann 12 Natural) (PrimV 'seq))
  (vector-set! store (ann 13 Natural) (PrimV 'make-array))
  (vector-set! store (ann 14 Natural) (PrimV 'array))
  (vector-set! store (ann 15 Natural) (PrimV 'aref))
  (vector-set! store (ann 16 Natural) (PrimV 'aset!))
  store)
...
(define (new-free [store : Store] [n : Natural]) : Natural
  (vector-set! store (ann 0 Natural) (NumV n))
  n)
...
(define (allocate [store : Store] [length : Natural]) : Natural
  ...
  (if (= length 0)
      ...
      (if (> new (vector-length store))
          ...
          (new-free store (cast new Nonnegative-Integer))
          ))
  base)
...
(define (parse [e : Sexp]) : ExprC
  (match e
    ...
    [(list (? symbol? id) ':= exp) (MutC
                                    (cast id Symbol) ; cast must succeed by definition of pattern above
                                    (cast (parse exp) ExprC))] ; cast must succeed by definition of pattern above
    [(? cnbt? (? symbol? s))
...
(define (primv-interp [sym : Symbol] [operands : (Listof Value)] [store : Store]) : Value
  (match (list sym operands)
    ...
    [(list '/ (list (NumV a) (NumV b))) (if (zero? b)
                                            (error 'primv-interp "SHEQ: can't do division by ~e" b) (NumV (/ a b)))]
    [(list '<= (list (NumV a) (NumV b))) (if (<= a b)
                                             (BoolV #t)
                                             (BoolV #f))]
    [(list 'strlen (list (StrV s))) (NumV (string-length s))]
    [(list 'equal? (list (ArrayV loc1 len1) (ArrayV loc2 len2))) (BoolV (= loc1 loc2))]
    ...
    [(list 'make-array (list (NumV s) fill))
     (if (exact-integer? s)
         (if (>= s 1)
             (let ([base (allocate store (cast s Natural))])
               (for ([i (in-range base (+ base s))])
                 (vector-set! store i fill))
               (ArrayV base (cast s Natural)))
    ...
    [(list 'array args) (define base (allocate store (length args)))
                        (for ([arg : Value (in-list args)]
                                  [i : Natural (in-naturals (cast base Nonnegative-Integer))])
                              (vector-set! store i arg))
                        (ArrayV (cast base Nonnegative-Integer) (length args))]
    ...
    [(list 'aset! (list (ArrayV location length) (NumV n) fill))
     (if (exact-integer? n)
         (if (and (>= n 0) (< n length))
             (vector-set! store (+ location (cast n Natural)) fill)
...
(define (interp [exp : ExprC] [env : Environment] [store : Store]) : Value
  (match exp
    ...
    [(AppC f a)
     ...
     (define arg-vals (map (lambda (arg) (interp (cast arg ExprC) env store)) a))
     (match fun
       [(CloV p b e)
        (if (= (length p) (length arg-vals))
            ...
            (error 'interp "SHEQ: wrong number of arguments: need ~e, got ~e" (length p) (length arg-vals)))]
...
(define in-order '{lambda (arr size) :
                    {let {[i = 0]
                          [ok = true]}
                      in
                      {seq
...
(define arr1 (primv-interp 'array (list (NumV 4) (NumV 13) (BoolV #f) (NumV 5)) store1))
(check-equal? (primv-interp 'aref (list arr1 (NumV 0)) store1) (NumV 4))
(check-equal? (primv-interp 'aref (list arr1 (NumV 2)) store1) (BoolV #f))
(check-equal? (primv-interp 'equal? (list arr1 arr) store1) (BoolV #f))
(check-equal? (primv-interp 'equal? (list arr arr) store1) (BoolV #t))
...
(check-exn (regexp (regexp-quote "SHEQ: cannot allocate 0 cells"))
           (lambda () (primv-interp  'array (list) store1 )))

(check-exn (regexp (regexp-quote "SHEQ: index out of bounds 8"))
           (lambda () (primv-interp  'aref (list arr1 (NumV 8)) store1)))
...
(check-equal? (primv-interp 'seq (list (BoolV #f) (NumV 12)) (vector (NumV 1) (BoolV #f))) (NumV 12))
(check-exn (regexp (regexp-quote "SHEQ: cannot allocate 0 cells"))
           (lambda () (primv-interp 'array '() (vector (NumV 1) (BoolV #f)))))
...
;interp
(check-equal? (interp (parse '{let {[x = 5]}
                                in
                                {let {[func = {lambda () : {x := 10}}]}
                                  in
                                  {func}
                                  end}
...
(check-equal? (interp (AppC (lambdaC '(x) (idC 'x)) (list (NumC 42))) top-env (make-initial-store 100)) (NumV 42))
...
(check-equal? (interp (AppC (lambdaC '(x y) (AppC (idC 'equal?) (list (idC 'x) (idC 'y))))
                            (list (strC "5") (NumC 6))) top-env (make-initial-store 100)) (BoolV #f))
(check-equal? (interp (AppC (lambdaC '(x y) (AppC (idC 'equal?) (list (idC 'x) (idC 'y))))
...
(check-equal? (primv-interp 'substring (list (StrV "apple") (NumV 2) (NumV 4)) (make-initial-store 100)) (StrV "pl"))
(check-equal? (primv-interp 'substring (list (StrV "apple") (NumV 2) (NumV 2)) (make-initial-store 100)) (StrV ""))
(check-equal? (primv-interp 'substring (list (StrV "apple") (NumV 0) (NumV 4)) (make-initial-store 100)) (StrV "appl"))
(check-equal? (primv-interp 'substring (list (StrV "apple") (NumV 0) (NumV 5)) (make-initial-store 100)) (StrV "apple"))
...
(check-exn (regexp (regexp-quote "SHEQ: wrong number of arguments"))
           (lambda () (interp (AppC (lambdaC '(x y) (idC 'x)) (list (NumC 1))) top-env (make-initial-store 100))))
...
(check-exn (regexp (regexp-quote "SHEQ: need numbers for +"))
           (lambda () (interp (AppC (lambdaC '() (AppC (idC '+) (list (strC "hi") (NumC 5)))) '())
...
(check-exn (regexp (regexp-quote "SHEQ: need a string to get the length"))
           (lambda () (interp (AppC (lambdaC '() (AppC (idC 'strlen) (list (NumC 5)))) '())
                              top-env (make-initial-store 100))))
...
(check-equal? (top-interp '{let {[x = "hello"] [y = 1] [z = 3]} in {substring x y z} end} 100) "\"el\"")

(check-exn (regexp (regexp-quote "SHEQ: user-error \"1234\""))
           (lambda () (top-interp '(+ 4 (error "1234")) 100)))
(check-exn (regexp (regexp-quote "SHEQ: errors expect 1 argument"))
           (lambda () (top-interp '(+ 4 (error "1234" "5678")) 100)))
